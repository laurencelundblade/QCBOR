 ...
 *
 * These fields provide extra binding to the sender and recipient that improve
 * security in some use cases. (To be honest, it is hard to decribe exactly what
 * these scenarios are and in the opinion of the t_cose author they might not be that
 * critical. The original motivation comes from Appendix B of NIST SP800-56A.)
 *
 * No option is provided for setting party U and V nonce because they are primarily
 * to provide randomness and the salt input to the HKDF, which is set by default, provides it. Similarly,
 * party U and V other seem unneeded. Theses values are all set to nil.
 */
static inline void
...
/**
 * Set the "SuppPubInfo.other" field of PartyInfo as described in RFC 9053.
 * This is optional and will be nil if not set. If this is set it will be sent to the recipient in a
 * header parameter.  Don't call this or pass NULL_USEFUL_BUF_C to not set this.
 *
 * Also sets "SuppPrivInfo" from PartyInfo. This is optional. It is never sent
 * since it is pivate info. Somehow the recipient must also know and set this
 * during decryption. Don't call this or pass NULL_USEFUL_BUF_C to not set this.
 *
 * The reasons for setting these and background on what to set it
 * to are in Section 5.2 of RFC 9053 and in NIST SP800-56A.
 */
static inline void
